...
export function SeriesHeader({ seriesNumber, seriesName, onEdit }: SeriesHeaderProps) {
  return (
    <header className="sticky top-0 z-10 flex items-center justify-center bg-background px-5 pb-4 pt-14">
      <h1 className="text-lg font-bold tracking-tight text-foreground text-balance text-center">
        {"Serie "}{seriesNumber}{" : "}{seriesName}
      </h1>
      <button
        onClick={onEdit}
        className="absolute right-5 top-14 flex h-10 w-10 items-center justify-center rounded-xl transition-colors hover:bg-secondary active:scale-95"
