...
import { getSeriesById, type VocabSeries } from "@/lib/data"
...
export function EditSeriesScreen({ id }: EditSeriesScreenProps) {
  const router = useRouter()
  const scrollRef = useRef<HTMLDivElement>(null)
  const lastRowRef = useRef<HTMLInputElement>(null)

  const [series, setSeries] = useState<VocabSeries | null>(null)
  ...
  const filledWordCount = words.filter(
    (w) => w.source.trim().length > 0 && w.target.trim().length > 0
  ).length
  ...
  const handleRemoveWord = useCallback((wordId: string | number) => {
    setWords((prev) => prev.filter((w) => w.id !== wordId))
  }, [])

  const handleSourceChange = useCallback(
    (wordId: string | number, value: string) => {
      setWords((prev) =>
        prev.map((w) => (w.id === wordId ? { ...w, source: value } : w))
      )
    },
    []
  )
  ...
  const handleTargetChange = useCallback(
    (wordId: string | number, value: string) => {
      setWords((prev) =>
        prev.map((w) => (w.id === wordId ? { ...w, target: value } : w))
      )
    },
    []
  )
  ...
  const handleSubmit = useCallback(() => {
    ...
    const updatedSeries = {
      ...
      words: validWords.map((w, i) => ({
        id: typeof w.id === "number" ? w.id : Date.now() + i,
    ...
    // For now, log the updated series (ready for future persistence)
    console.log("Updated series:", updatedSeries)
    router.push("/")
  }, [canSubmit, words, name, sourceLanguage, targetLanguage, router, series])
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 z-20 bg-background">
          <div className="px-5 pt-14 pb-4 flex items-start gap-2">
            <button
              onClick={() => router.push("/")}
              className="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-xl transition-colors hover:bg-secondary active:scale-95"
              aria-label="Retour"
            >
